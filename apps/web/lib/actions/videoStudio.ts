...
function buildWorkflow(): WorkflowDefinition {
  ...
  return {
    startNodeId: "open_chatgpt", variables: {}, edges: [], nodes: [
...
export async function runIdeaToFlowVideo(formData: FormData) {
  ...
  workflow.publishedVersionId = version._id; await workflow.save();
  const automation = await Automation.create({ name: `Flow Video - ${idea.slice(0, 48)}`, description: "One-idea video automation using logged-in ChatGPT and Google Flow sessions; no paid AI API required.", slug: `flow-video-${Date.now().toString(36)}`, workflowId: workflow._id, browserProfileId, status: "active", enabled: true });
  const task = await Task.create({ automationId: automation._id, workflowId: workflow._id, workflowVersionId: version._id, status: "QUEUED", input: { idea }, browserProfileId, source: "dashboard" });
