...
export interface BrowserSessionJobData {
...
export const automationTaskQueue = () => getQueue<AutomationTaskJobData>(QUEUE_NAMES.automationTasks);
...
export async function enqueueAutomationTask(taskId: string, opts?: { priority?: number; delay?: number }): Promise<void> {
  await automationTaskQueue().add(
    "run-task",
    { taskId },
    {
      priority: opts?.priority,
      delay: opts?.delay,
      attempts: 1, // retries are handled inside the workflow engine per-node, not by BullMQ
